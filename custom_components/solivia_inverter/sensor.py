...
#   - these are the only illegal chars found in the solivia DATA_TABLE
...
class MyCoordinator(DataUpdateCoordinator):
    def __init__(self, hass):
        super().__init__(hass, _LOGGER, name="Solivia Inverter", update_interval=timedelta(seconds=60))
...
class MyEntity(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, idx):
        ...
        self._attr_device_class                 = DATA_TABLE[idx][4]
        self._attr_native_unit_of_measurement   = DATA_TABLE[idx][5]
        self._attr_state_class                  = DATA_TABLE[idx][6]
        
        self._attr_unique_id = f"{DOMAIN}_{sanitize_str(self._attr_name)}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, "My Awesome Inverter")},
            name="Solivia Inverter",
            manufacturer="Delta",
            model="EU G3",
        )
